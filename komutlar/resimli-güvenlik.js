...
let logkanal = await db.fetch(`guvenlik${message.guild.id}`)
  ...
  if (args[0] === "sıfırla" || args[0] === "kapat") {
    if(!logkanal) return message.channel.send(`Güvenliği kapatmak için \`güvenlik kanalının\` seçili olması lazım örnek kullanım: \`z!güvenlik #kanal\``);
...
if (!logk) return message.channel.send('Güvenlik kanalını bulamadım  Kullanım `z!güvenlik #kanal`');
